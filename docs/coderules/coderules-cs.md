...
- **ALL CODE MUST PASS `dotnet format --verify-no-changes` AND ANALYSER CHECKS** before committing

## Test Locations

- **CRITICAL**: Tests MUST be placed in a separate test project (e.g., `ProjectName.Tests`)
- Use xUnit, NUnit, or MSTest consistently throughout the project
- Test classes should mirror the structure of the source project
- Integration tests go in a separate project (e.g., `ProjectName.IntegrationTests`)

## Developing in C#

- Never do "fallback" programming in terms of requirements: if you expect a dependency, fail immediately if it's not present
- Use the latest stable .NET version and C# language features (C# 10+)
- Backwards compatibility is NOT a goal, neither in terms of .NET versions, nor in terms of this project's code itself
- **Nullable reference types**: Enable `<Nullable>enable</Nullable>` in all projects; treat null warnings as errors
- Use file-scoped namespaces to reduce nesting
- Use primary constructors and records where appropriate
- Prefer pattern matching over type checking and casting

## Code Quality

- **CRITICAL**: Run `dotnet format` before committing
- Enable and respect analyser warnings (treat as errors in CI)
...
- Consider the Result pattern for expected failure cases in library code
...
- Use `Debug.WriteLine()` or logging for quick debugging, but ensure debug code is not committed
- Use a proper logging framework (Serilog, NLog, or Microsoft.Extensions.Logging) for production needs
- Use IDE breakpoints and the debugger for complex issues
...
- We practice TDD:
  ...
  - Review any skipped tests (`[Skip]`, `[Ignore]`) and ensure they are documented.
  - Fix any compiler and analyser warnings.
