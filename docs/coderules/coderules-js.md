...
- **CRITICAL**: Tests MUST be placed in `tests/` or alongside source files as `*.test.js` / `*.spec.js`
- Use a consistent testing framework (Jest, Vitest, or Mocha)
- Integration/E2E tests go in `tests/e2e/` or `tests/integration/`

## Developing in JavaScript

- Never do "fallback" programming in terms of requirements: if you expect a module, fail immediately if it's not present
- **CRITICAL**: No dynamic imports for core dependencies. All imports at the top of files only
  - WRONG: `const foo = await import('bar')` inside a function for static dependencies
  - CORRECT: All imports at file top
  - Dynamic imports are acceptable only for genuine code-splitting scenarios
- Use modern ES6+ syntax: arrow functions, destructuring, template literals, optional chaining
- Use ES modules (`import`/`export`) over CommonJS (`require`/`module.exports`) where possible
- Backwards compatibility is NOT a goal, neither in terms of Node.js/browser versions, nor in terms of this project's code itself
- **Type safety**:
  - Prefer TypeScript for new projects
  - If using plain JavaScript, use JSDoc type annotations for public APIs
  - Enable strict mode (`'use strict'` or ES modules)

## Code Quality

- **CRITICAL**: Run ESLint before committing; treat all warnings as errors
- Run Prettier (or equivalent formatter) before committing
- Avoid `var`; use `const` by default, `let` only when reassignment is needed
- Avoid `any` type in TypeScript; use `unknown` and narrow appropriately
- Use `===` and `!==` over `==` and `!=`
- Handle Promise rejections; never leave unhandled rejections
- Avoid callback hell; prefer async/await over raw Promises where it improves readability

## Error Handling

- Use `try/catch` for async/await error handling
- Throw `Error` objects, not strings or other primitives
- Provide meaningful error messages with context
- Don't silently swallow errors; log or rethrow

## Debugging

- **CRITICAL**: Always identify root causes of failures. Do NOT treat the symptoms of failures.
- Use `console.log()` or `console.debug()` for quick debugging, but remove before committing
- Use a proper logging library for production logging needs
- Use the debugger statement or IDE breakpoints for complex issues
...
- We practice TDD:
  ...
  - Review any skipped tests (`.skip`) and ensure they are documented.
